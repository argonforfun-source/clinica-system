...
const targetListener = `  // Load Doctors
  db.ref(BASE+'/doctors').on('value', snap => {
    _docs = snap.val()||{};`;

const replaceListener = `  // Load Staff
  let _staff = {};
...
if (content.includes(targetListener) && !content.includes("db.ref(BASE+'/staff').on('value'")) {
    content = content.replace(targetListener, replaceListener);
