...
(function (global) {
  ...
  function printChart() {
    ...
    var time = new Date().toLocaleTimeString('ar-JO', { hour: '2-digit', minute:'2-digit' });
    ...
    var html = [
      ...
      '<head>',
        '<meta charset="UTF-8">',
        '<title>تقرير الأسنان السريري - ' + _esc(patientName) + '</title>',
        '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">',
        '<style>',
          '@import url("https://fonts.googleapis.com/css2?family=Tajawal:wght@400;700;900&display=swap");',
          'body { font-family: "Tajawal", sans-serif; padding: 0; margin: 0; background: #fff; color: #0f172a; -webkit-print-color-adjust: exact; print-color-adjust: exact; }',
          '.print-container { max-width: 100%; margin: 0 auto; padding: 40px; }',
          '.print-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 3px solid #0ea5e9; padding-bottom: 20px; margin-bottom: 30px; }',
          '.clinic-logo { display: flex; align-items: center; gap: 15px; }',
          '.clinic-logo i { font-size: 40px; color: #0ea5e9; }',
          '.clinic-brand h2 { margin: 0; color: #0f172a; font-size: 28px; font-weight: 900; letter-spacing: -0.5px; }',
          '.clinic-brand p { margin: 5px 0 0; color: #64748b; font-size: 14px; }',
          '.report-title { text-align: left; }',
          '.report-title h1 { margin: 0; font-size: 22px; color: #0f172a; text-transform: uppercase; letter-spacing: 1px; }',
          '.report-title p { margin: 5px 0 0; color: #64748b; font-size: 14px; }',
          '.patient-info-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; background: #f8fafc; padding: 20px; border-radius: 12px; border: 1px solid #e2e8f0; margin-bottom: 30px; }',
          ...
          '.info-val { font-size: 16px; color: #0f172a; font-weight: bold; }',
          '.print-section { margin-bottom: 40px; }',
          '.print-section-title { font-size: 18px; color: #0ea5e9; border-bottom: 2px solid #e0f2fe; padding-bottom: 10px; margin-bottom: 20px; display: flex; align-items: center; gap: 10px; }',
          '.footer-signatures { margin-top: 60px; display: grid; grid-template-columns: 1fr 1fr; gap: 50px; text-align: center; }',
          '.sig-box { padding-top: 20px; border-top: 1px dashed #94a3b8; color: #0f172a; font-weight: bold; }',
          '@media print {',
          '  body { padding: 0; }',
        ...
        '<div class="print-container">',
          '<div class="print-header">',
            '<div class="clinic-logo">',
              '<i class="fas fa-tooth" style="font-size: 40px; color: #0ea5e9;"></i>',
              '<div class="clinic-brand" style="margin-right: 15px;">',
                '<h2 style="margin: 0; color: #0f172a; font-size: 28px; font-weight: 900;">ARGON EMR</h2>',
                '<p style="margin: 5px 0 0; color: #64748b; font-size: 14px;">نظام إدارة العيادات الذكي والمتقدم</p>',
              '</div>',
            '</div>',
            '<div class="report-title">',
              '<h1 style="margin: 0; font-size: 22px; color: #0f172a; letter-spacing: 1px;">تقرير فحص الأسنان</h1>',
              '<p style="margin: 5px 0 0; color: #64748b; font-size: 14px; text-align: left;">Dental Examination Report</p>',
            '</div>',
          '</div>',

          '<div class="patient-info-grid" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; background: #f8fafc; padding: 20px; border-radius: 12px; border: 1px solid #e2e8f0; margin-bottom: 30px;">',
            '<div class="info-box" style="display: flex; flex-direction: column; gap: 5px;"><span class="info-label" style="font-size: 12px; color: #64748b; font-weight: bold;">اسم المريض</span><span class="info-val" style="font-size: 16px; color: #0f172a; font-weight: bold;">' + _esc(patientName) + '</span></div>',
            '<div class="info-box" style="display: flex; flex-direction: column; gap: 5px;"><span class="info-label" style="font-size: 12px; color: #64748b; font-weight: bold;">رقم الملف (ID)</span><span class="info-val" style="font-size: 16px; color: #0f172a; font-weight: bold;">' + (_currentPatientId || 'غير محدد') + '</span></div>',
            '<div class="info-box" style="display: flex; flex-direction: column; gap: 5px;"><span class="info-label" style="font-size: 12px; color: #64748b; font-weight: bold;">تاريخ التقرير</span><span class="info-val" style="font-size: 16px; color: #0f172a; font-weight: bold;">' + date + ' - ' + new Date().toLocaleTimeString('ar-JO', {hour:'2-digit', minute:'2-digit'}) + '</span></div>',
          '</div>',
          
          '<div class="print-section">',
            '<h3 class="print-section-title" style="font-size: 18px; color: #0ea5e9; border-bottom: 2px solid #e0f2fe; padding-bottom: 10px; margin-bottom: 20px;">المخطط السريري والتشخيص المفصل</h3>',
            '<div>' + chartContent + '</div>',
          '</div>',
          
          '<div class="footer-signatures" style="margin-top: 60px; display: grid; grid-template-columns: 1fr 1fr; gap: 50px; text-align: center;">',
            '<div class="sig-box" style="padding-top: 20px; border-top: 1px dashed #94a3b8; font-weight: bold;">توقيع المريض / Patient Signature</div>',
            '<div class="sig-box" style="padding-top: 20px; border-top: 1px dashed #94a3b8; font-weight: bold;">توقيع الطبيب المعالج / Doctor Signature</div>',
          '</div>',
          
          '<div style="text-align: center; margin-top: 40px; font-size: 12px; color: #94a3b8;">',
            'تم إنشاء هذا التقرير إلكترونياً عبر نظام Argon EMR &copy; - غير صالح للاستخدام القانوني دون التواقيع المعتمدة.',
          '</div>',
        '</div>',
