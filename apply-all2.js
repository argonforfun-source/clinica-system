...
if (idx3 !== -1) {
    ...
    // Regex fallback
    const regexBlock = /blockAccess:\s*function\(reason\)\s*\{[\s\S]*?document\.body\.innerHTML[\s\S]*?<\/div>[\s\S]*?`;?\s*\}/m;
    if (content.match(regexBlock)) {
        content = content.replace(regexBlock, blockAccessReplace);
        console.log("3. Injected Enterprise Login UI via Regex");
    } else {
        console.log("3. Could not match blockAccess");
    }
}
...
const clearRegex = /clear:\s*function\(\)\s*\{\s*sessionStorage\.removeItem\(this\.KEY\);\s*window\.location\.reload\(\);\s*\}/m;
if (content.match(clearRegex)) {
    content = content.replace(clearRegex, `clear: function() {
        sessionStorage.removeItem(this.KEY);
    }`);
    console.log("6. Fixed ArgonSessionRegistry.clear() by removing reload");
} else {
    console.log("6. Could not find clearRegex");
}
...
if (content.match(isoRegex)) {
    ...
    console.log("7. Could not match isolation regex");
}
