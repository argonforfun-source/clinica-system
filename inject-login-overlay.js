...
const targetStr = `    blockAccess: function(reason) {
...
const replaceStr = `    blockAccess: function(reason, allowedRoles) {
        ArgonSessionRegistry.clear();
        
        // If it's a role mismatch (user logged in but wrong portal), show Access Denied
        if (reason && reason.includes('الوصلاحية')) {
             document.body.innerHTML = \`
               <div style="height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#0f172a;color:white;font-family:'Tajawal',sans-serif;">
                  <i class="fas fa-shield-alt" style="font-size:4rem;color:#ef4444;margin-bottom:20px;"></i>
                  <h1 style="margin:0 0 10px 0;">الوصول مرفوض</h1>
    ...
    injectEnterpriseLoginUI: function(allowedRoles) {
        ...
        overlay.innerHTML = \`
          <div style="background:var(--surf, #fff); padding:40px; border-radius:24px; box-shadow:0 10px 40px rgba(0,0,0,0.05); width:100%; max-width:400px; text-align:center; border:1px solid var(--border, #e2e8f0);">
             ...
             <button id="entLoginBtn" style="width:100%; padding:14px; background:var(--teal, #0d9488); color:#fff; border:none; border-radius:12px; font-size:1.1rem; font-weight:bold; cursor:pointer; transition:0.2s; font-family:inherit; box-shadow:0 4px 15px rgba(13,148,136,0.3);">
                تسجيل الدخول الدخول <i class="fas fa-arrow-left" style="margin-right:8px;"></i>
             </button>
          </div>
        \`;
        ...
        document.getElementById('entLoginBtn').onclick = async () => {
            ...
            if (!staffId) return toast('يرجى اختيار الحساب', 'err');
            if (!pwd) return toast('يرجى إدخال كلمة المرور', 'err');
            ...
            try {
                ...
                // Success! Audit and reload gracefully
                ...
                btn.innerHTML = 'تسجيل الدخول الدخول <i class="fas fa-arrow-left" style="margin-right:8px;"></i>';
                btn.disabled = false;
                toast(e.message, 'err');
            }
        };
...
// Also update authorizePortal to pass allowedRoles to blockAccess
const authTargetStr = `               this.blockAccess("الجلسة غير صالحة أو غير موجودة. يرجى تسجيل الدخول مجدداً.");`;
const authReplaceStr = `               this.blockAccess("الجلسة غير صالحة أو غير موجودة. يرجى تسجيل الدخول مجدداً.", allowedRoles);`;

if (content.includes(targetStr)) {
    content = content.replace(targetStr, replaceStr);
    content = content.replace(authTargetStr, authReplaceStr);
    fs.writeFileSync(path, content, 'utf8');
    console.log("Injected injectEnterpriseLoginUI successfully.");
} else {
    // Maybe encoding mismatch?
    console.log("Could not find target blockAccess");
}
