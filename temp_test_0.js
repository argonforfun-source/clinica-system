...
if (uP.get('id')) localStorage.setItem('argon_id', uP.get('id'));
  ...
  dg.innerHTML=keys.map(k=>{
    ...
    return `<div class="dc">
      ...
      <div style="display:flex;border-top:1px solid rgba(13,148,136,.1)">
          <button onclick="openSecModal('${k}', 'doctors')" style="flex:1;background:none;border:none;border-left:1px solid rgba(13,148,136,.1);color:var(--teal);padding:8px;font-family:'Tajawal',sans-serif;font-size:.75rem;cursor:pointer;transition:.2s"><i class="fas fa-shield-alt"></i> الأمان</button>
...
// Redeploy trigger: 2026-05-18T21:08:12
  ...
  function renderSecurityHub(data) {
    ...
    window.renderStaffManagement = function(data) {
  ...
  let _activeSecType = 'doctors'; // 'doctors' or 'staff'
      ...
      html += `
        <tr style="border-bottom:1px solid rgba(255,255,255,0.02); transition:0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.02)'" onmouseout="this.style.background='transparent'">
          ...
          <td style="padding:15px 12px; text-align:center;">
            <button onclick="openSecModal('${k}', 'doctors')" style="background:var(--teal); color:#000; border:none; padding:6px 12px; border-radius:6px; font-family:inherit; font-weight:700; cursor:pointer; font-size:0.8rem; transition:0.2s"><i class="fas fa-key"></i> تعيين الباسورد</button>
...
if('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js').catch(()=>{});
  ...
  window.toggleComplexView = toggleComplexView;
  window.renderDashboardDeptFilters = renderDashboardDeptFilters;
  window.setDashboardDeptFilter = setDashboardDeptFilter;
  window.renderDepts = renderDepts;
  window.updateDocDeptDropdown = updateDocDeptDropdown;
  window.addDept = addDept;
  window.deleteDept = deleteDept;
  window.recalcBookings = recalcBookings;
  window.ring = ring;
  window.renderLive = renderLive;
  window.recalcCmp = recalcCmp;
  window.openSecurityModal = openSecurityModal;
  window.closeSecurityModal = closeSecurityModal;
  window.secForceReset = secForceReset;
  window.secSetTempPass = secSetTempPass;
  window.updSt = updSt;
  window.cmpBook = cmpBook;
  window.clrHist = clrHist;
  window.addDoc = addDoc;
  window.editDoc = editDoc;
  window.cancelDocEdit = cancelDocEdit;
  window.handleImageUpload = handleImageUpload;
  window.togAvail = togAvail;
  window.saveSettings = saveSettings;
  window.copyPatPortalUrl = copyPatPortalUrl;
  ...
  window.upLogo = upLogo;
  window.delLogo = delLogo;
  window.toast = toast;
  window.initSecurityHubListener = initSecurityHubListener;
  window.renderSecurityHub = renderSecurityHub;
  window.openSecModal = openSecModal;
  window.closeSecModal = closeSecModal;
  window.secSetPassword = secSetPassword;
  window.secRevokeSessions = secRevokeSessions;
  window.secToggleSuspend = secToggleSuspend;
  window.secInvalidateSession = secInvalidateSession;
