...
function generatePatientFileHTML(uid) {
  ...
  const fileHTML = `
    <div class="pat-card">
      ...
      <div class="pat-grid">
        <div class="pat-field"><div class="pfl">رقم الهاتف</div><div class="pfv    ${window.ArgonSpecialtyLoader && window.ArgonSpecialtyLoader.hasFeature('dentalChart') ? `
  ...
  return fileHTML;ab' ? 'active' : ''}" onclick="switchEmrTab('dental-chart-tab')" style="background:var(--surf);border:1px solid var(--border);color:var(--muted);padding:8px 16px;border-radius:10px;font-family:'Tajawal',sans-serif;font-weight:700;font-size:0.85rem;cursor:pointer;display:inline-flex;align-items:center;gap:6px;transition:all 0.2s">
        <i class="fas fa-tooth" style="color:#3b82f6"></i> 🦷 الرسم البياني للأسنان
      </button>` : ''}
    </div>
