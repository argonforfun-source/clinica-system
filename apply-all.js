...
const blockAccessTarget = `blockAccess: function(reason) {
...
// First we must find the exact blockAccess function.
...
const loginTarget = `async login(doctorId, password, clinicId) {
    if (!firebase || !firebase.database) throw new Error("Firebase not ready");
    await this.ensureAnonAuth();
    const db = firebase.database();
    
    // 1. Fetch Credentials
    const credSnapshot = await db.ref(\`clinics/\${clinicId}/doctors/\${doctorId}/credentials\`).once('value');`;

const loginReplace = `async login(doctorId, password, clinicId, type = 'doctors') {
    if (!firebase || !firebase.database) throw new Error("Firebase not ready");
    await this.ensureAnonAuth();
    const db = firebase.database();
...
if (content.includes(loginTarget)) {
    ...
    console.log("5. Updated ArgonEnterpriseAuth.login successfully");
} else {
    // try line by line match
    content = content.replace(/async login\(doctorId, password, clinicId\) \{/g, `async login(doctorId, password, clinicId, type = 'doctors') {`)
                     .replace(/await db\.ref\(\`clinics\/\$\{clinicId\}\/doctors\/\$\{doctorId\}\/credentials\`\)\.once\('value'\);/g, `await db.ref(\`clinics/\${clinicId}/\${type}/\${doctorId}/credentials\`).once('value');`);
    console.log("5. Updated ArgonEnterpriseAuth.login via regex");
}
...
/*
    clear: function() {
        ...
        // Do not reload automatically, let the caller handle UI logout
    },
*/
// The second one is:
/*
...
*/
const clearTarget = `clear: function() {
        sessionStorage.removeItem(this.KEY);
        window.location.reload();
    }`;
const clearReplace = `clear: function() {
        sessionStorage.removeItem(this.KEY);
    }`;
if (content.includes(clearTarget)) {
    content = content.replace(clearTarget, clearReplace);
    console.log("6. Fixed ArgonSessionRegistry.clear() by removing reload");
}

// 7. Enforce Isolation in SecurityGateway.filterAccessiblePatients
const isolationTarget = `static filterAccessiblePatients(patients, session) {
      if (!session) return [];
      ...
      if (session.role === 'admin' || session.role === 'reception') return patients;
      
      return patients.filter(([uid, p]) => {
         const info = p.info || {};
         const visibilityScope = info.visibilityScope || 'CLINIC_SHARED';
         
         // Ownership Enforcement Rule (V8.3)
         if (visibilityScope === 'PRIVATE' || info.ownershipLocked === true) {
            return info.assignedDoctorId === session.staffId;
         }
         
         // V8.3 Enterprise Shared Rule: Exclude deleted or explicitly hidden
         if (info.isDeleted === true) return false;
...
if (content.includes(isolationTarget)) {
    content = content.replace(isolationTarget, isolationReplace);
    console.log("7. Enforced Strict Isolation in filterAccessiblePatients");
} else {
    // maybe line endings differ
    const regex = /static filterAccessiblePatients\(patients, session\) \{[\s\S]*?return true;\s*\}\);\s*\}/m;
    if (content.match(regex)) {
        content = content.replace(regex, isolationReplace);
        console.log("7. Enforced Strict Isolation via regex");
    } else {
        console.log("7. Could not match isolation target");
    }
}
