...
// 3. Remove Security Hub from Settings!
const securityHubTargetRegex = /<!-- ENTERPRISE SECURITY HUB -->[\s\S]*?<div style="margin-top:15px; background:rgba\(239,68,68,0\.05\); padding:10px; border-radius:8px; display:flex; gap:10px; align-items:center;">/m;

// Wait, I will just remove the whole Enterprise Security Hub block since we will use openSecurityModal for staff cards instead of a unified table!
// Actually, let me just find it and remove it.
const startHub = content.indexOf('<!-- ENTERPRISE SECURITY HUB -->');
if (startHub !== -1) {
    // Find the end of this block. It ends right before `</div>` that closes the `.setp` or something?
    // Let's use a simpler string replace if we can.
    // The Security Hub was injected right after `<div class="setg"><label class="setl">هاتف العيادة</label>...</div>`
}

fs.writeFileSync(path, content, 'utf8');
console.log("Injected Staff tab HTML into dashboard.html");
